...
export const ListDetail: React.FC = () => {
  ...
  if (!isLoading && !currentList) {
    return <div className="p-8 text-center">书单不存在或已被删除。</div>;
  }
  ...
  return (
    <div>
      {/* 头部导航与信息 */}
      <div className="mb-6">
        <Link to="/" className="inline-flex items-center text-slate-500 hover:text-primary mb-4 transition">
          <ChevronLeftIcon className="w-4 h-4 mr-1" />
          返回书单列表
        </Link>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 mb-2">{currentList?.title}</h1>
